...
- **`bot/main.py`** wires everything and runs `web.run_app`. **`build_app` is synchronous on purpose**: all async setup (SQLite `store.init()`, `set_webhook`, the periodic prune task) happens inside aiohttp `on_startup`, so everything lives on `web.run_app`'s event loop. Do **not** open the aiosqlite connection on a different loop (e.g. via `run_until_complete` before `run_app`) — it fails at runtime with "attached to a different loop". `set_webhook` in startup is wrapped in try/except so a bad token logs-and-continues instead of aborting startup.
